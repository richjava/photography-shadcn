...
function runUpdateCommand(includePlugins = false, retryCount = 0) {
  ...
  updateTimeout = setTimeout(() => {
    isUpdating = true;
    const command = includePlugins ? 'npx create-built-app update' : 'npx create-built-app update --noPlugins';
    console.log(`Running update command: ${command}`);
    
    try {
      const updateProcess = spawn('npx', ['create-built-app', 'update', includePlugins ? '' : '--noPlugins'], {
        stdio: 'inherit'
      });
